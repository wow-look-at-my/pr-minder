...
- **The base is the branch's fork point by default** (`base_from_fork_point`, on). pr-minder opens the PR into the branch the head was forked from — the nearest ancestor commit that is another branch's HEAD — so a branch forked from a non-default branch targets *that* branch rather than the default branch. **By default any branch can be the base** (a tie at the same commit prefers the default branch); when no other branch's tip is found in the history it falls back to `target_base` (or the default branch). `base_branch_patterns` optionally **restricts** which branches may be a base (only the default branch or a pattern match) — useful for a version-branch layout. Set `"base_from_fork_point": false` to always open into `target_base` instead.
