...
- **The base is the branch's fork point by default** (`base_from_fork_point`, on). pr-minder opens the PR into the branch the head was forked from, so a branch forked from a non-default branch targets *that* branch rather than the default branch. **By default any branch can be the base** (the nearest fork point wins; a tie at the same commit prefers the default branch); when no fork point is found it falls back to `target_base` (or the default branch). It still finds the parent **even if that branch has moved on since you forked** — gained commits after the fork, so the fork commit is no longer its tip — by checking the parent's recent history, not just its tip. `base_branch_patterns` optionally **restricts** which branches may be a base (only the default branch or a pattern match) — useful for a version-branch layout, or to keep the history check focused in a very large repo. Set `"base_from_fork_point": false` to always open into `target_base` instead.
