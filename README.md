...
- **The base is the branch's fork point by default** (`base_from_fork_point`, on). pr-minder opens the PR into the branch the head was forked from — the nearest ancestor commit that is another branch's HEAD — so a branch forked from a non-default branch targets *that* branch rather than the default branch. Non-default branches qualify as a base only if they match `base_branch_patterns` (the default branch always qualifies); when no fork point is found it falls back to `target_base` (or the default branch). Set `"base_from_fork_point": false` to always open into `target_base` instead.
