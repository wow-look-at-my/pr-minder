...
Day to day, this is driven by the `labeled` webhook — adding the label acts immediately. As a safety net, the periodic fleet-wide reconciliation (the [`pr-minder-reconcile`](https://github.com/wow-look-at-my/webhooks) hook) **searches every installation for labeled-but-unarmed PRs every few minutes** and arms (or merges) them. So a PR labeled while an older/buggy version was running, or whose webhook was dropped, heals automatically within minutes — without polling the API on every event.
...
```jsonc
{
  ...
  "auto_open_pr": {
    ...
    // Optional: fallback base when fork-point detection finds nothing (defaults to the repo's default branch).
    "target_base": "",
...
When a branch is ahead of its base and has no open PR, pr-minder opens one (`head` = the branch, `base` = the branch's fork point — the branch it was created from — falling back to `target_base` or the default branch, title = the branch name). It runs on two triggers:

- **On a push to a non-default branch** — opens the PR the moment the branch gets a commit, going forward.
- **The periodic fleet-wide reconciliation** — sweeps existing branches (typically within the hour), so branches that predate the App, or whose push webhook was missed, also get PRs.

Notes:
- **Opt-in**, off by default. The default branch and `gh-pages` are always skipped; add more via `skip_branches`.
- **The base is the branch's fork point by default** (`base_from_fork_point`, on). pr-minder opens the PR into the branch the head was forked from, so a branch forked from a non-default branch targets *that* branch rather than the default branch. **By default any branch can be the base**: the branch whose fork point (merge base) is nearest to the head wins, and the **default branch is always evaluated and wins every tie** — two branches pointing at the same fork commit are interchangeable parents, so the canonical one is chosen, and a stale sibling pointer parked at your fork commit can never hijack the PR base away from the default branch. When no fork point is found it falls back to `target_base` (or the default branch). It still finds the parent **even if that branch has moved on since you forked** — gained commits after the fork, so the fork commit is no longer its tip — by checking the parent's recent history, not just its tip (the default branch is checked with one exact comparison, so it is found however far it has advanced). `base_branch_patterns` optionally **restricts** which branches may be a base (only the default branch or a pattern match) — useful for a version-branch layout, or to keep the history check focused in a very large repo. Set `"base_from_fork_point": false` to always open into `target_base` instead.
