...
```
src/
  worker.ts     Entry point: Cloudflare Worker. fetch(): GET/HEAD -> serveDocs(), POST -> webhook; on each isolate's first request it kicks startupReconcile() once via ctx.waitUntil (reconcile-on-startup, not polling). scheduled(): the cron entry point -> runRechecks() (drains the deferred-revive reminders)
  ...
  handlers.ts   Event dispatch: handle(), onPR(), onPushToDefault(), onPushToBranch(), onInstallation(), onReposAdded(), prQualifies(), isActionsBotPr(), shouldConsiderRevive(), reviveIfZombie(), runRechecks(), maybeBackfillRepo(), startupReconcile(), reconcileAutoMerge(), shouldSkipBranch()
  ...
  state.ts      Workers KV access: per-PR "checked at SHA" markers + per-repo backfill flag + per-PR "recheck later" reminders (checkedSha/markChecked, wasBackfilled/markBackfilled, setRecheck/clearRecheck/listRechecks)
...
wrangler.toml             Worker name, compat date, [build] gzip step, [[rules]] Data for *.gz, [[kv_namespaces]] PR_STATE, [version_metadata] CF_VERSION_METADATA (deploy id for the once-per-deploy startup gate), [triggers] crons (the every-5-min recheck sweep), plain vars (GITHUB_APP_ID)
```
...
- **Cron, every 5 min** → `runRechecks` (the Worker's `scheduled` handler). Drains the `recheck:` reminders left by deferred follow-up commits, re-evaluating each PR once it has aged. This is **not a poll over all PRs**: it reads only the reminders (`listRechecks`, a KV list), so with none pending it makes a single KV call and **zero GitHub API calls**. Per pending PR it mints a token for that repo (`repoInstallationId` → `installToken`, cached per run), refetches the PR (`getPull`), and re-runs `reviveIfZombie`. Closed/merged PRs clear their reminder; a missing token or transient fetch failure leaves it for the next sweep (KV TTL ~1 day bounds an abandoned one).
...
Auto-merge reconcile (also no cron, no polling — startup + install only): `reconcileAutoMerge(repo, config, token)` re-scans a repo's open PRs and (re)enables auto-merge on any that carry an `auto_merge`-mode label but aren't armed (`enableAutoMerge` direct-merges a PR that's already mergeable, so a "ready" PR gets merged instead of sitting forever). It pre-filters on the `auto_merge`/`labels`/`draft` fields already in the `listOpenPulls` payload, so it only spends an `enableAutoMerge` call on a labeled-but-unarmed PR. It runs in exactly two places: (1) **Worker startup** — `startupReconcile(env)` sweeps **every** installation (`listInstallations` → per-install token → `listInstallationRepos` → `reconcileAutoMerge`), so a redeploy heals PRs whose auto-merge state drifted under an older/buggy version. It's fired from `worker.ts` on each isolate's first request (`ctx.waitUntil`, guarded by a module flag = once per isolate), then **gated once per deploy** by a KV flag keyed on the Worker version (`startup:{CF_VERSION_METADATA.id}` in `PR_STATE`): the first isolate to claim the flag sweeps, the rest skip — so a redeploy triggers exactly one cross-repo sweep instead of one per edge isolate. The flag is claimed *before* sweeping so a concurrent isolate that loses the race bails; with no version binding it degrades to the per-isolate guard (a constant key would block all future deploys). (2) **install / repos-added** — `reconcileAutoMerge` runs alongside the other sweeps in `onInstallation`/`onReposAdded`. Steady state is driven entirely by the live `labeled`/`unlabeled`/`auto_merge_enabled`/`auto_merge_disabled` webhooks — there is deliberately **no** per-event re-scan (that would hammer the API; the webhook already says what changed).
