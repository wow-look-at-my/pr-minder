...
- Live `pull_request` **opened / reopened / synchronize** → `reviveIfZombie` on that PR, gated by `shouldConsiderRevive(action, sender)`. `synchronize` is included so new commits get re-checked, but **only when `github-actions[bot]` pushed them** — it's the sole actor whose commits skip CI. A `synchronize` from anyone else (a human, a third-party app, or pr-minder's **own `update-branch` merge**) produced a commit that triggers CI natively, so it's not re-revived; this is what stops an auto-updated bot PR from being closed+reopened a *second* time (the `update-branch` merge changes the head SHA, so KV hasn't deduped it, and its runs haven't registered yet, so `hasWorkflowRuns` momentarily reads 0 — which used to trip a needless revive). Reopens we sent ourselves (`sender.type === 'Bot'`) are skipped so close+reopen can't loop; if it reopened, `onPR` returns and the fresh event drives the rest.
