...
wrangler.toml             Worker name, compat date, [build] gzip step, [[rules]] Data for *.gz, [[kv_namespaces]] PR_STATE, [version_metadata] CF_VERSION_METADATA (deploy id for the once-per-deploy startup gate), plain vars (GITHUB_APP_ID)
```
