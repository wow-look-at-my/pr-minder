...
`auto_trigger_workflows`: boolean (default false). Revives "zombie" PRs with no workflow runs by closing+reopening them (`retriggerWorkflows` in `github.ts`), so a fresh `pull_request.reopened` event triggers their CI. The zombie test (`needsWorkflowTrigger`) differs by event, plus a catch-up sweep at onboarding:
...
- `installation`/`installation_repositories` — a catch-up sweep (`maybeRetriggerZombiesForRepo`) lists every open PR (`listOpenPulls`, paginated) and reopens the zombies, the retroactive complement to the live `opened`/`reopened` paths (which only act on PRs going forward). Revives pre-existing zombies at onboarding without waiting for a manual reopen. Runs **before** the `auto_open_pr` sweep so a PR opened that same pass (App-authored, CI fires natively) isn't mistaken for a zombie by its momentary zero runs. Each PR's retrigger is wrapped in try/catch so one failure doesn't abort the rest. **Rate-limit shape**: `listOpenPulls` already carries each PR's author, so the sweep gates on `isActionsBotPr` (free) and spends the one `hasWorkflowRuns` call ONLY on bot-authored PRs — the sole GITHUB_TOKEN-zombie kind — so cost is ~1 API call per *bot-authored* open PR, not per open PR, bounding the burst when a large installation is onboarded (App installation tokens get 5,000–12,500 req/hr). Drafts and PRs with no head SHA are skipped. Unlike the author-agnostic `reopened` path, the sweep is bot-author-only by design; a human/PAT PR with zero runs means the repo has no PR CI (a reopen can't fix that), and a human can still reopen any single PR to hit the author-agnostic path.
- `push` to the **default branch** — the same `maybeRetriggerZombiesForRepo` sweep also runs inside `onPushToDefault`. This is the ambient trigger for **already-installed** repos: GitHub never re-sends an install event for an onboarded repo, so without it a pre-existing zombie would sit dead until a manual reopen or an app re-add. Instead it self-heals on the repo's next default-branch push. Self-limiting: once a revived PR's CI runs it has runs, so the next push's sweep skips it (no flapping in the normal case; a repo with no PR workflows at all is unfixable regardless).

Loop safety: our own close+reopen (live path or sweep) returns as a `reopened` event whose `sender.type` is `Bot`; reopens from any bot sender are skipped, so the sweep can't loop either. `hasWorkflowRuns` fails safe to `true` (assume runs exist) on a query error, so transient failures never cause a spurious reopen.
...
- `push` handling: default-branch pushes → `onPushToDefault` (zombie sweep for already-installed repos via `maybeRetriggerZombiesForRepo`, then update qualifying PRs); non-default branch pushes → `onPushToBranch` (`auto_open_pr`). Both gated on `ref` being `refs/heads/*` and `!p.deleted`, so tag pushes and branch deletions are ignored. A PR pr-minder opens is authored by the App's own bot, so it triggers CI natively and the `auto_trigger_workflows` zombie check (which keys on `github-actions[bot]`) correctly ignores it — no interaction between the two features
